export const provinces = [
  { id: 1, nombre: 'Distrito Nacional / Santo Domingo' },
  { id: 2, nombre: 'Santiago' },
  { id: 3, nombre: 'Azua' },
  { id: 4, nombre: 'Bahoruco' },
  { id: 5, nombre: 'Barahona' },
  { id: 6, nombre: 'Dajabón' },
  { id: 7, nombre: 'Duarte' },
  { id: 8, nombre: 'Elías Piña' },
  { id: 9, nombre: 'El Seibo' },
  { id: 10, nombre: 'Espaillat' },
  { id: 11, nombre: 'Independencia' },
  { id: 12, nombre: 'La Altagracia' },
  { id: 13, nombre: 'La Romana' },
  { id: 14, nombre: 'La Vega' },
  { id: 15, nombre: 'María Trinidad Sánchez' },
  { id: 16, nombre: 'Monte Cristi' },
  { id: 17, nombre: 'Pedernales' },
  { id: 18, nombre: 'Peravia' },
  { id: 19, nombre: 'Puerto Plata' },
  { id: 20, nombre: 'Hermanas Mirabal' },
  { id: 21, nombre: 'Samaná' },
  { id: 22, nombre: 'San Cristóbal' },
  { id: 23, nombre: 'San Juan de la Maguana' },
  { id: 24, nombre: 'San Pedro de Macorís' },
  { id: 25, nombre: 'Sánchez Ramírez' },
  { id: 26, nombre: 'Santiago Rodríguez' },
  { id: 27, nombre: 'Valverde' },
  { id: 28, nombre: 'Monseñor Nouel' },
  { id: 29, nombre: 'Monte Plata' },
  { id: 30, nombre: 'Hato Mayor' },
  { id: 31, nombre: 'San José de Ocoa' },
];

export const municipalities = [
  {
    id: 1,
    provinciaId: 1,
    nombre: 'Santo Domingo de Guzmán',
  },
  {
    id: 2,
    provinciaId: 3,
    nombre: 'Azua',
  },
  {
    id: 3,
    provinciaId: 3,
    nombre: 'Las Charcas',
  },
  {
    id: 4,
    provinciaId: 3,
    nombre: 'Las Yayas de Viajama',
  },
  {
    id: 5,
    provinciaId: 3,
    nombre: 'Padre Las Casas',
  },
  {
    id: 6,
    provinciaId: 3,
    nombre: 'Peralta',
  },
  {
    id: 7,
    provinciaId: 3,
    nombre: 'Sabana Yegua',
  },
  {
    id: 8,
    provinciaId: 3,
    nombre: 'Pueblo Viejo',
  },
  {
    id: 9,
    provinciaId: 3,
    nombre: 'Tábara Arriba',
  },
  {
    id: 10,
    provinciaId: 3,
    nombre: 'Guayabal',
  },
  {
    id: 11,
    provinciaId: 3,
    nombre: 'Estebanía',
  },
  {
    id: 12,
    provinciaId: 4,
    nombre: 'Neiba',
  },
  {
    id: 13,
    provinciaId: 4,
    nombre: 'Galván',
  },
  {
    id: 14,
    provinciaId: 4,
    nombre: 'Tamayo',
  },
  {
    id: 15,
    provinciaId: 4,
    nombre: 'Villa Jaragua',
  },
  {
    id: 16,
    provinciaId: 4,
    nombre: 'Los Ríos',
  },
  {
    id: 17,
    provinciaId: 5,
    nombre: 'Barahona',
  },
  {
    id: 18,
    provinciaId: 5,
    nombre: 'Cabral',
  },
  {
    id: 19,
    provinciaId: 5,
    nombre: 'Enriquillo',
  },
  {
    id: 20,
    provinciaId: 5,
    nombre: 'Paraíso',
  },
  {
    id: 21,
    provinciaId: 5,
    nombre: 'Vicente Noble',
  },
  {
    id: 22,
    provinciaId: 5,
    nombre: 'El Peñón',
  },
  {
    id: 23,
    provinciaId: 5,
    nombre: 'La Ciénaga',
  },
  {
    id: 24,
    provinciaId: 5,
    nombre: 'Fundación',
  },
  {
    id: 25,
    provinciaId: 5,
    nombre: 'Las Salinas',
  },
  {
    id: 26,
    provinciaId: 5,
    nombre: 'Polo',
  },
  {
    id: 27,
    provinciaId: 5,
    nombre: 'Jaquimeyes',
  },
  {
    id: 28,
    provinciaId: 6,
    nombre: 'Dajabón',
  },
  {
    id: 29,
    provinciaId: 6,
    nombre: 'Loma de Cabrera',
  },
  {
    id: 30,
    provinciaId: 6,
    nombre: 'Partido',
  },
  {
    id: 31,
    provinciaId: 6,
    nombre: 'Restauración',
  },
  {
    id: 32,
    provinciaId: 6,
    nombre: 'El Pino',
  },
  {
    id: 33,
    provinciaId: 7,
    nombre: 'San Francisco de Macorís',
  },
  {
    id: 34,
    provinciaId: 7,
    nombre: 'Arenoso',
  },
  {
    id: 35,
    provinciaId: 7,
    nombre: 'Castillo',
  },
  {
    id: 36,
    provinciaId: 7,
    nombre: 'Pimentel',
  },
  {
    id: 37,
    provinciaId: 7,
    nombre: 'Villa Riva',
  },
  {
    id: 38,
    provinciaId: 7,
    nombre: 'Las Guáranas',
  },
  {
    id: 39,
    provinciaId: 7,
    nombre: 'Eugenio María de Hostos',
  },
  {
    id: 40,
    provinciaId: 8,
    nombre: 'Comendador',
  },
  {
    id: 41,
    provinciaId: 8,
    nombre: 'Bánica',
  },
  {
    id: 42,
    provinciaId: 8,
    nombre: 'El Llano',
  },
  {
    id: 43,
    provinciaId: 8,
    nombre: 'Hondo Valle',
  },
  {
    id: 44,
    provinciaId: 8,
    nombre: 'Pedro Santana',
  },
  {
    id: 45,
    provinciaId: 8,
    nombre: 'Juan Santiago',
  },
  {
    id: 46,
    provinciaId: 9,
    nombre: 'El Seibo',
  },
  {
    id: 47,
    provinciaId: 9,
    nombre: 'Miches',
  },
  {
    id: 48,
    provinciaId: 10,
    nombre: 'Moca',
  },
  {
    id: 49,
    provinciaId: 10,
    nombre: 'Cayetano Germosén',
  },
  {
    id: 50,
    provinciaId: 10,
    nombre: 'Gaspar Hernández',
  },
  {
    id: 51,
    provinciaId: 10,
    nombre: 'Jamao Al Norte',
  },
  {
    id: 52,
    provinciaId: 10,
    nombre: 'San Víctor ',
  },
  {
    id: 53,
    provinciaId: 11,
    nombre: 'Jimaní',
  },
  {
    id: 54,
    provinciaId: 11,
    nombre: 'Duvergé',
  },
  {
    id: 55,
    provinciaId: 11,
    nombre: 'La Descubierta',
  },
  {
    id: 56,
    provinciaId: 11,
    nombre: 'Postrer Río',
  },
  {
    id: 57,
    provinciaId: 11,
    nombre: 'Cristóbal',
  },
  {
    id: 58,
    provinciaId: 11,
    nombre: 'Mella',
  },
  {
    id: 59,
    provinciaId: 12,
    nombre: 'Higüey',
  },
  {
    id: 60,
    provinciaId: 12,
    nombre: 'San Rafael del Yuma',
  },
  {
    id: 61,
    provinciaId: 13,
    nombre: 'La Romana',
  },
  {
    id: 62,
    provinciaId: 13,
    nombre: 'Guaymate',
  },
  {
    id: 63,
    provinciaId: 13,
    nombre: 'Villa Hermosa',
  },
  {
    id: 64,
    provinciaId: 14,
    nombre: 'La Vega',
  },
  {
    id: 65,
    provinciaId: 14,
    nombre: 'Constanza',
  },
  {
    id: 66,
    provinciaId: 14,
    nombre: 'Jarabacoa',
  },
  {
    id: 67,
    provinciaId: 14,
    nombre: 'Jima Abajo',
  },
  {
    id: 68,
    provinciaId: 15,
    nombre: 'Nagua',
  },
  {
    id: 69,
    provinciaId: 15,
    nombre: 'Cabrera',
  },
  {
    id: 70,
    provinciaId: 15,
    nombre: 'El Factor',
  },
  {
    id: 71,
    provinciaId: 15,
    nombre: 'Río San Juan',
  },
  {
    id: 72,
    provinciaId: 16,
    nombre: 'Monte Cristi',
  },
  {
    id: 73,
    provinciaId: 16,
    nombre: 'Castañuelas',
  },
  {
    id: 74,
    provinciaId: 16,
    nombre: 'Guayubín',
  },
  {
    id: 75,
    provinciaId: 16,
    nombre: 'Las Matas de Santa Cruz',
  },
  {
    id: 76,
    provinciaId: 16,
    nombre: 'Pepillo Salcedo',
  },
  {
    id: 77,
    provinciaId: 16,
    nombre: 'Villa Vásquez',
  },
  {
    id: 78,
    provinciaId: 17,
    nombre: 'Pedernales',
  },
  {
    id: 79,
    provinciaId: 17,
    nombre: 'Oviedo',
  },
  {
    id: 80,
    provinciaId: 18,
    nombre: 'Baní',
  },
  {
    id: 81,
    provinciaId: 18,
    nombre: 'Nizao',
  },
  {
    id: 82,
    provinciaId: 18,
    nombre: 'Matanzas',
  },
  {
    id: 83,
    provinciaId: 19,
    nombre: 'Puerto Plata',
  },
  {
    id: 84,
    provinciaId: 19,
    nombre: 'Altamira',
  },
  {
    id: 85,
    provinciaId: 19,
    nombre: 'Guananico',
  },
  {
    id: 86,
    provinciaId: 19,
    nombre: 'Imbert',
  },
  {
    id: 87,
    provinciaId: 19,
    nombre: 'Los Hidalgos',
  },
  {
    id: 88,
    provinciaId: 19,
    nombre: 'Luperón',
  },
  {
    id: 89,
    provinciaId: 19,
    nombre: 'Sosúa',
  },
  {
    id: 90,
    provinciaId: 19,
    nombre: 'Villa Isabela',
  },
  {
    id: 91,
    provinciaId: 19,
    nombre: 'Villa Montellano',
  },
  {
    id: 92,
    provinciaId: 20,
    nombre: 'Salcedo',
  },
  {
    id: 93,
    provinciaId: 20,
    nombre: 'Tenares',
  },
  {
    id: 94,
    provinciaId: 20,
    nombre: 'Villa Tapia',
  },
  {
    id: 95,
    provinciaId: 21,
    nombre: 'Samaná',
  },
  {
    id: 96,
    provinciaId: 21,
    nombre: 'Sánchez',
  },
  {
    id: 97,
    provinciaId: 21,
    nombre: 'Las Terrenas',
  },
  {
    id: 98,
    provinciaId: 22,
    nombre: 'San Cristóbal',
  },
  {
    id: 99,
    provinciaId: 22,
    nombre: 'Sabana Grande de Palenque',
  },
  {
    id: 100,
    provinciaId: 22,
    nombre: 'Bajos de Haina',
  },
  {
    id: 101,
    provinciaId: 22,
    nombre: 'Cambita Garabitos',
  },
  {
    id: 102,
    provinciaId: 22,
    nombre: 'Villa Altagracia',
  },
  {
    id: 103,
    provinciaId: 22,
    nombre: 'Yaguate',
  },
  {
    id: 104,
    provinciaId: 22,
    nombre: 'San Gregorio de Nigua',
  },
  {
    id: 105,
    provinciaId: 22,
    nombre: 'Los Cacaos',
  },
  {
    id: 106,
    provinciaId: 23,
    nombre: 'San Juan',
  },
  {
    id: 107,
    provinciaId: 23,
    nombre: 'Bohechío',
  },
  {
    id: 108,
    provinciaId: 23,
    nombre: 'El Cercado',
  },
  {
    id: 109,
    provinciaId: 23,
    nombre: 'Juan de Herrera',
  },
  {
    id: 110,
    provinciaId: 23,
    nombre: 'Las Matas de Farfán',
  },
  {
    id: 111,
    provinciaId: 23,
    nombre: 'Vallejuelo',
  },
  {
    id: 112,
    provinciaId: 24,
    nombre: 'San Pedro de Macorís',
  },
  {
    id: 113,
    provinciaId: 24,
    nombre: 'Los Llanos',
  },
  {
    id: 114,
    provinciaId: 24,
    nombre: 'Ramón Santana',
  },
  {
    id: 115,
    provinciaId: 24,
    nombre: 'Consuelo',
  },
  {
    id: 116,
    provinciaId: 24,
    nombre: 'Quisqueya',
  },
  {
    id: 117,
    provinciaId: 24,
    nombre: 'Guayacanes',
  },
  {
    id: 118,
    provinciaId: 25,
    nombre: 'Cotuí',
  },
  {
    id: 119,
    provinciaId: 25,
    nombre: 'Cevicos',
  },
  {
    id: 120,
    provinciaId: 25,
    nombre: 'Fantino',
  },
  {
    id: 121,
    provinciaId: 25,
    nombre: 'La Mata',
  },
  {
    id: 122,
    provinciaId: 25,
    nombre: 'Santiago',
  },
  {
    id: 123,
    provinciaId: 2,
    nombre: 'Bisonó',
  },
  {
    id: 124,
    provinciaId: 2,
    nombre: 'Jánico',
  },
  {
    id: 125,
    provinciaId: 2,
    nombre: 'Licey al Medio',
  },
  {
    id: 126,
    provinciaId: 2,
    nombre: 'San José de Las Matas',
  },
  {
    id: 127,
    provinciaId: 2,
    nombre: 'Tamboril',
  },
  {
    id: 128,
    provinciaId: 2,
    nombre: 'Villa González',
  },
  {
    id: 129,
    provinciaId: 2,
    nombre: 'Puñal',
  },
  {
    id: 130,
    provinciaId: 2,
    nombre: 'Sabana Iglesia ',
  },
  {
    id: 131,
    provinciaId: 2,
    nombre: 'Baitoa',
  },
  {
    id: 132,
    provinciaId: 26,
    nombre: 'San Ignacio de Sabaneta',
  },
  {
    id: 133,
    provinciaId: 26,
    nombre: 'Villa Los Almácigos',
  },
  {
    id: 134,
    provinciaId: 26,
    nombre: 'Monción',
  },
  {
    id: 135,
    provinciaId: 27,
    nombre: 'Mao',
  },
  {
    id: 136,
    provinciaId: 27,
    nombre: 'Esperanza',
  },
  {
    id: 137,
    provinciaId: 27,
    nombre: 'Laguna Salada',
  },
  {
    id: 138,
    provinciaId: 28,
    nombre: 'Bonao',
  },
  {
    id: 139,
    provinciaId: 28,
    nombre: 'Maimón',
  },
  {
    id: 140,
    provinciaId: 28,
    nombre: 'Piedra Blanca',
  },
  {
    id: 141,
    provinciaId: 29,
    nombre: 'Monte Plata',
  },
  {
    id: 142,
    provinciaId: 29,
    nombre: 'Bayaguana',
  },
  {
    id: 143,
    provinciaId: 29,
    nombre: 'Sabana Grande de Boyá',
  },
  {
    id: 144,
    provinciaId: 29,
    nombre: 'Yamasá',
  },
  {
    id: 145,
    provinciaId: 29,
    nombre: 'Peralvillo',
  },
  {
    id: 146,
    provinciaId: 30,
    nombre: 'Hato Mayor',
  },
  {
    id: 147,
    provinciaId: 30,
    nombre: 'Sabana de La Mar',
  },
  {
    id: 148,
    provinciaId: 30,
    nombre: 'El Valle',
  },
  {
    id: 149,
    provinciaId: 31,
    nombre: 'San José de Ocoa',
  },
  {
    id: 150,
    provinciaId: 31,
    nombre: 'Sabana Larga',
  },
  {
    id: 151,
    provinciaId: 31,
    nombre: 'Rancho Arriba',
  },
  {
    id: 152,
    provinciaId: 1,
    nombre: 'Santo Domingo Este',
  },
  {
    id: 153,
    provinciaId: 1,
    nombre: 'Santo Domingo Oeste',
  },
  {
    id: 154,
    provinciaId: 1,
    nombre: 'Santo Domingo Norte',
  },
  {
    id: 155,
    provinciaId: 1,
    nombre: 'Boca Chica',
  },
  {
    id: 156,
    provinciaId: 1,
    nombre: 'San Antonio de Guerra',
  },
  {
    id: 157,
    provinciaId: 1,
    nombre: 'Los Alcarrizos',
  },
  {
    id: 158,
    provinciaId: 1,
    nombre: 'Pedro Brand',
  },
  {
    id: 159,
    provinciaId: 2,
    nombre:'Santiago de los caballeros'
  },

];
